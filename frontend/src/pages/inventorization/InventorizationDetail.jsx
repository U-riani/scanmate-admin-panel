...
export default function InventorizationDetail() {
  ...
  const employees = doc.employees.map((eid) => pocketUsers.find((u) => u.id === eid)).filter(Boolean);
  ...
  function createRecount() {
    ...
    recountMutation.mutate({ parent_document_id: doc.id, warehouse_id: doc.warehouse_id, employees: doc.employees, items: recountItems });
    setSelectedLines([]);
  }
  ...
  return (
    <div className="space-y-5">
      <ImportInventorizationExcelModal open={importOpen} onClose={() => setImportOpen(false)} documentId={doc.id} />
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="page-title">
            Inventorization{" "}
            <span className="font-mono" style={{ color: "var(--accent-cyan)", fontSize: "1.1rem" }}>#{doc.id}</span>
          </h1>
      ...
      <div className="glass-card p-5">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8">
          <div className="info-row"><span className="info-label">Warehouse</span><span className="info-value">{warehouse?.name ?? "—"}</span></div>
          <div className="info-row"><span className="info-label">Type</span><span className="info-value">{doc.type}</span></div>
          ...
          <div className="info-row"><span className="info-label">Employees</span><span className="info-value">{employees.map((e) => e.username).join(", ") || "—"}</span></div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div className="glass-card p-5">
          <div className="flex justify-between items-center mb-3">
            ...
            <span
              className="font-mono"
              style={{ color: "var(--accent-cyan)", fontSize: "1.25rem", fontWeight: 700 }}
            >
              {progress}%
            </span>
          </div>
      ...
      <div className="glass-card p-5">
        <div className="flex items-center justify-between mb-4">
          <h2 style={{ fontWeight: 600, fontSize: "0.9375rem" }}>Inventorization Lines</h2>
          ...
          <table className="glass-table">
            ...
            <tbody>
              {lines.length === 0 ? (
                ...
                lines.map((line) => {
                  const diff = line.counted_qty === null ? null : line.counted_qty - line.expected_qty;
                  return (
                    <tr key={line.id}>
                      <td>
                        <input
